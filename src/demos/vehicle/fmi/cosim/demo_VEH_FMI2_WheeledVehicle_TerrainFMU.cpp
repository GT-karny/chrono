...
void CreateTerrainFMU(FmuChronoUnit& terrain_fmu,
                      const std::string& instance_name,
                      const std::string& fmu_filename,
                      const std::string& fmu_unpack_dir,
                      double step_size,
                      ...
                      const std::string& out_path) {
    try {
        terrain_fmu.Load(fmi2Type::fmi2CoSimulation, fmu_filename, fmu_unpack_dir);
    } catch (std::exception&) {
        throw;
    }
    std::cout << "Terrain FMU version:  " << terrain_fmu.GetVersion() << std::endl;
    std::cout << "Terrain FMU platform: " << terrain_fmu.GetTypesPlatform() << std::endl;

    // Instantiate FMU
    try {
        terrain_fmu.Instantiate(instance_name);
    } catch (std::exception&) {
        throw;
    }

    // Set debug logging
    terrain_fmu.SetDebugLogging(fmi2True, logCategories);

    // Initialize FMU
    terrain_fmu.SetupExperiment(fmi2False, 0.0,         // define tolerance
                                start_time,             // start time
                                fmi2False, stop_time);  // use stop time
    // Set I/O fixed parameters
    terrain_fmu.SetVariable("out_path", out_path);

    // Set fixed parameters
    // For this demo, we can choose "Flat" or "RigidMesh"
    // Example: Use flat terrain
    std::string terrain_type = "Flat"; 
    double friction = 0.8;
    
    terrain_fmu.SetVariable("terrain_type", terrain_type);
    terrain_fmu.SetVariable("friction", friction, FmuVariable::Type::Real);
    terrain_fmu.SetVariable("step_size", step_size, FmuVariable::Type::Real);
}
...
int main(int argc, char* argv[]) {
    ...
    std::string terrain_fmu_model_identifier = "FMU2cs_Terrain";
    ...
    std::string terrain_fmu_dir = CHRONO_VEHICLE_FMU_DIR + terrain_fmu_model_identifier + std::string("/");
    ...
    std::string terrain_fmu_filename = terrain_fmu_dir + terrain_fmu_model_identifier + std::string(".fmu");
#else
    // Expect fully qualified FMU filenames as program arguments
    if (argc != 6) {
        std::cout << "Usage: ./demo_VEH_FMI2_WheeledVehicle_TerrainFMU [vehicle_FMU] [powertrain_FMU] [driver_FMU] [tire_FMU] [terrain_FMU]"
                  << std::endl;
        return 1;
    }
    ...
    std::string terrain_fmu_filename = argv[5];
#endif
    ...
    std::string terrain_unpack_dir = CHRONO_VEHICLE_FMU_DIR + std::string("tmp_unpack_terrain/");
    ...
    std::string terrain_instance_name = "TerrainFMU";

    // Create (if needed) output directories
    std::string out_dir = GetChronoOutputPath() + "./DEMO_WHEELEDVEHICLE_FMI_TERRAIN";
    ...
    std::string terrain_out_dir = out_dir + "/" + terrain_instance_name;
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (!filesystem::create_directory(filesystem::path(terrain_out_dir + "_" + std::to_string(i)))) {
            std::cout << "Error creating directory " << terrain_out_dir + "_" + std::to_string(i) << std::endl;
            return 1;
        }
    }
    ...
    for (int i = 0; i < 4; i++) {
        ...
        try {
             CreateTerrainFMU(terrain_fmu[i],
                              terrain_instance_name + "_" + std::to_string(i), terrain_fmu_filename, terrain_unpack_dir,
                              step, start_time, stop_time,
                              logCategories, terrain_out_dir + "_" + std::to_string(i));
        } catch (std::exception& e) {
            std::cout << "ERROR loading terrain FMU: " << e.what() << "\n";
            return 1;
        }
    }
    ...
    for (int i = 0; i < 4; i++) {
        ...
        terrain_fmu[i].EnterInitializationMode();
        terrain_fmu[i].ExitInitializationMode();
    }
    ...
    while (time < t_end) {
        ...
        // ----------- Exchange data between tire and terrain FMUs
        for (int i = 0; i < 4; i++) {
            WheelState state;
            ...
            // --- Tire <-> Terrain integration ---
            ChVector3d query_point;
            tire_fmu[i].GetVecVariable("query_point", query_point);
            
            terrain_fmu[i].SetVecVariable("query_point", query_point);
            
            // Step Terrain FMU to calculate outputs
            terrain_fmu[i].DoStep(time, step, fmi2True);
            
            double h, mu;
            ChVector3d n;
            terrain_fmu[i].GetVariable("height", h, FmuVariable::Type::Real);
            terrain_fmu[i].GetVecVariable("normal", n);
            terrain_fmu[i].GetVariable("mu", mu, FmuVariable::Type::Real);
            
            tire_fmu[i].SetVariable("terrain_height", h, FmuVariable::Type::Real);
            tire_fmu[i].SetVecVariable("terrain_normal", n);
            tire_fmu[i].SetVariable("terrain_mu", mu, FmuVariable::Type::Real);
        }
        ...
        // Terrain FMU already stepped in loop
